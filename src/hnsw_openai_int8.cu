...
int main(int argc, char *argv[])
{
...

    {
        ...
        cuvs::preprocessing::quantize::scalar::quantizer<float> q = cuvs::preprocessing::quantize::scalar::train(res, params,
                                                                                                                 dataset.view());
        auto quantized_dataset = raft::make_host_matrix<int8_t, int64_t>(dataset.rows(), dataset.cols());

        cuvs::preprocessing::quantize::scalar::transform(res, q, dataset.view(), quantized_dataset.view());

        const char *quant_path = "quantized_dataset.i8bin";
        std::ofstream ofs(quant_path, std::ios::binary | std::ios_base::out);
        if (!ofs)
            throw std::runtime_error(std::string("Failed to open for write: ") + quant_path);
        uint32_t dims[2] = {static_cast<uint32_t>(quantized_dataset.extent(0)),
                            static_cast<uint32_t>(quantized_dataset.extent(1))};
        ofs.write(reinterpret_cast<const char *>(dims), 8);
        ofs.write(reinterpret_cast<const char *>(quantized_dataset.data_handle()),
                  quantized_dataset.size() * sizeof(int8_t));
        std::cout << "Quantized dataset saved: " << quant_path << std::endl;
        ...
        std::cout << "Dataset quantized in " << avg_time_seconds << " seconds" << std::endl;
    }

    BinaryFile<int8_t> quantized_dataset("quantized_dataset.i8bin", max_dataset_rows);
    std::cout << "Dataset shape: [" << quantized_dataset.rows() << ", " << quantized_dataset.cols() << "]" << std::endl;
    auto start_time = std::chrono::high_resolution_clock::now();
    ...
    auto index_params =
        cagra::index_params::from_hnsw_params(quantized_dataset.view().extents(),
                                              M,
    ...
    auto cagra_index = cagra::build(res, index_params, quantized_dataset.view());

    cagra::index<float, uint32_t> cagra_f_index(res, params.metric);
    cagra_f_index.update_graph(res, cagra_index.graph());

    // Convert CAGRA index to HNSW
    std::cout << "Converting CAGRA index to HNSW" << std::endl;
    auto hnsw_index = hnsw::from_cagra(res, params, cagra_f_index, dataset.view());
